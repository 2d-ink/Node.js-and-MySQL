...
var promptManager=function(res){
    inquirer.prompt([{
        ...
        choices:["Add new", "Add quantity to an existing item"]
    }]).then(function(val){
...
function addItem(){
    inquirer.prompt([{
        ...
        connection.query("INSERT INTO product(productname,departmentname,price,stockquantity) VALUES ('"+val.productname+"','"+val.departmentname+"',"+val.price+","+val.quantity+");", function(err,res){
            if(err)throw err;
    ...
    inquirer.prompt([{
        ...
        for(var i=0; i<res.length;i++){
            if(res[i].productname==val.productname){
                connection.query("UPDATE prodcuts SET stockquantity=stockquantity"+val.added+" Where itemid="+res[i].itemid+';', function(err,res){
                    if(err)throw err;
